...
const promptIntro = "You are an advanced medical assistant tasked with helping healthcare professionals assess thoracic diseases and conditions. Based on the patient information provided, analyze the symptoms and history to: Estimate the most likely diagnosis and provide a percentage likelihood for each suspected disease. Suggest alternative diagnoses that should be considered. Recommend additional examinations, diagnostic tests, or medical referrals necessary to confirm or rule out the suspected conditions. If the information is inconclusive, suggest questions or further information that could aid diagnosis.\n";
...
const [formData, setFormData] = useState({});
  ...
  const handleSubmit = (e) => {
    ...
    console.log(formattedData); // Replace with GPT completion call if needed.
    const finalPrompt = `${promptIntro}${formattedData}`;
  ...
  <div className="cd-report-questions">
    ...
    <input type="number" name="patientAge" required onChange={handleChange} />

    <label>2. What is the patient’s sex?</label>
    <div>
      <input type="radio" name="sex" value="male" required onChange={handleChange} /> Male
      <input type="radio" name="sex" value="female" required onChange={handleChange} /> Female
    </div>

    <label>3. What is the patient’s weight? (in kg)</label>
    <input type="number" name="patientWeight" required onChange={handleChange} />

    <label>4. What is the patient’s height? (in cm)</label>
    <input type="number" name="patientHeight" required onChange={handleChange} />

    <label>5. Does the patient have a fever?</label>
    <div>
      <input type="radio" name="fever" value="yes" required onChange={handleChange} /> Yes
      <input type="radio" name="fever" value="no" required onChange={handleChange} /> No
    </div>
